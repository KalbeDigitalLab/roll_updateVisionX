-- Patient Merge Cleanup (batched, safe, with progress NOTIFY)
-- Satu batch = satu transaksi. Gagal hanya rollback batch saat ini.
-- Progress: NOTIFY 'cleanup_progress' dengan payload JSON { batch, processed, total, pct }
...
BEGIN
  -- Session settings (apply in each transaction)
  SET work_mem = '256MB';

  -- Indexes (IF NOT EXISTS, aman dijalankan berulang)
  ...
  LOOP
    -- 1) Build list of pat_ids that need fix (in current transaction)
    ...
    -- 4) Canonical per pat_id for this batch
    ...
    -- 6) Reattach patient_id to canonical + elvasoft
    ...
    -- 7) Deduplicate only affected patient_id rows (bukan full table)
    WITH affected_fk AS (
    ...
    -- 8) Remove leftover non-elvasoft for this batch
    DELETE FROM public.patient_id pid
    USING _canonical c
    WHERE pid.pat_id = c.pat_id
      AND COALESCE(pid.entity_id,'') <> 'elvasoft';

    -- 9) Recompute num_studies for affected patients
    ...
    -- 10) Delete only truly unreferenced patients (step 6)
    ...
    -- 6b: Remove from _del_patients any pk still referenced by any FK to patient
    <<block6b>>
    DECLARE
      r RECORD;
    BEGIN
    ...
    -- Commit or Rollback (safety: only this batch is committed)
    IF p_dry_run THEN
      ROLLBACK;
      ...
      RETURN; -- DRY RUN: only one batch, then exit
    ELSE
      COMMIT;
    END IF;
...
-- Run cleanup: batch 500, LIVE (commit each batch)
CALL public.patient_merge_cleanup_batched(500, false);
