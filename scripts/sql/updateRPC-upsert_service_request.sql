...
BEGIN
  ...
  IF existing_id IS NOT NULL THEN
    ...
    SET
      ...
      "locationCode" = (SELECT ext ->> 'valueString'
                       ...
                       WHERE ext IS NOT NULL LIMIT 1),
      "hasStockOpname" = (SELECT (ext ->> 'valueBoolean')::boolean
  ...
  ELSE
    INSERT INTO public."serviceRequest" (
      id, modality, bookmarks, "isExported", "locationCode", "hasStockOpname",
      identifier, status, code, priority, subject, "occurrence.dateTime",
      requester, performer, reason, insurance, note,
      "performer.display", "requester.display", "subject.display",
