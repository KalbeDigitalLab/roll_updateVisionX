-- ============================================================
-- Migration: v12_fix_timezone_date_range_rpc
-- Updates fast_fetch_studies_paginated_filter_v6 date range filtering:
--   1. Casts incoming ISO start/end date strings with offset (e.g. +07:00) to timestamptz AT TIME ZONE 'UTC'
--   2. Fixes date boundary comparisons against stored UTC timestamps
-- ============================================================

CREATE OR REPLACE FUNCTION public.fast_fetch_studies_paginated_filter_v6(
  ...
  IF v_filters != '{}'::jsonb THEN
    ...
    IF v_filters->>'examination' IS NOT NULL AND jsonb_typeof(v_filters->'examination') = 'object' AND (v_filters->'examination'->>'type') = 'dateRange' THEN
      IF v_filters->'examination'->>'start' IS NOT NULL THEN
        v_where_clause := v_where_clause || ' AND sr."occurrence.dateTime" >= (' || quote_literal(v_filters->'examination'->>'start') || '::timestamptz AT TIME ZONE ''UTC'')';
      END IF;
      IF v_filters->'examination'->>'end' IS NOT NULL THEN
        v_where_clause := v_where_clause || ' AND sr."occurrence.dateTime" <= (' || quote_literal(v_filters->'examination'->>'end') || '::timestamptz AT TIME ZONE ''UTC'')';
      END IF;
    END IF;
    ...
    IF v_filters->>'examRead' IS NOT NULL THEN
      IF jsonb_typeof(v_filters->'examRead') = 'object' AND (v_filters->'examRead'->>'type') = 'dateRange' THEN
        IF v_filters->'examRead'->>'start' IS NOT NULL THEN
          v_where_clause := v_where_clause || ' AND dr."effective.dateTime" >= (' || quote_literal(v_filters->'examRead'->>'start') || '::timestamptz AT TIME ZONE ''UTC'')';
        END IF;
        IF v_filters->'examRead'->>'end' IS NOT NULL THEN
          v_where_clause := v_where_clause || ' AND dr."effective.dateTime" <= (' || quote_literal(v_filters->'examRead'->>'end') || '::timestamptz AT TIME ZONE ''UTC'')';
        END IF;
    ...
    IF v_filters->>'examRegister' IS NOT NULL THEN
      IF jsonb_typeof(v_filters->'examRegister') = 'object' AND (v_filters->'examRegister'->>'type') = 'dateRange' THEN
        IF v_filters->'examRegister'->>'start' IS NOT NULL THEN
          v_where_clause := v_where_clause || ' AND proc."performedDateTime" >= (' || quote_literal(v_filters->'examRegister'->>'start') || '::timestamptz AT TIME ZONE ''UTC'')';
        END IF;
        IF v_filters->'examRegister'->>'end' IS NOT NULL THEN
          v_where_clause := v_where_clause || ' AND proc."performedDateTime" <= (' || quote_literal(v_filters->'examRegister'->>'end') || '::timestamptz AT TIME ZONE ''UTC'')';
        END IF;
  ...
  v_sql := '
    ...
    FROM (
      SELECT jsonb_build_object(
        ...
        ''hasStockOpname'', COALESCE((sr."hasStockOpname")::boolean, false),
        ...
        ''serviceRequest'', jsonb_build_object(
          ...
          ''id'', sr.id,
          ''extension'',
            -- ★ v6: added hasStockOpname to CASE condition and UNION ALL
            CASE
              WHEN sr.modality IS NOT NULL OR sr.bookmarks IS NOT NULL
                OR sr."isExported" IS NOT NULL OR sr."locationCode" IS NOT NULL
                OR sr."hasStockOpname" IS NOT NULL
              THEN (
                ...
                FROM (
                  ...
                  UNION ALL
                  -- ★ v6: hasStockOpname extension
                  SELECT jsonb_build_object(
                    ''url'', ''http://example.org/fhir/StructureDefinition/hasStockOpname'',
                    ''valueBoolean'', COALESCE((sr."hasStockOpname")::boolean, false)
                  ) AS ext
                  WHERE TRUE
                ) extensions
              )
